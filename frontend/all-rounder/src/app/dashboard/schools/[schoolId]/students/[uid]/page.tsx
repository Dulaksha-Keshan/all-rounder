...
export default function StudentProfilePage({ params }: StudentProfilePageProps) {
  ...
  useEffect(() => {
    ...
    console.log("Fetching student profile for UID:", uid);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--page-bg)]">
        <div className="text-center">
          ...
          <p className="text-[var(--text-main)] font-semibold">Loading student profile...</p>
        </div>
      </div>
    );
  }
  ...
  if (!student || viewedUserRole !== "STUDENT") {
    notFound();
  }

  const school = getSchoolById(schoolId);
  const age = student.date_of_birth
    ? Math.max(0, new Date().getFullYear() - new Date(student.date_of_birth).getFullYear())
    : null;
      ...
      <div className="max-w-4xl mx-auto mt-6">
        ...
        <div className="bg-[var(--white)] rounded-xl shadow-lg p-8 border border-[var(--gray-200)] mb-6 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6 z-10">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-[var(--text-muted)] font-medium mb-1">{student.email}</p>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm mb-4">
                {student.grade && (
                  <span className="flex items-center gap-2 px-3 py-2 bg-blue-50 text-blue-700 rounded-lg font-medium">
                    📚 Grade {student.grade}
                  </span>
                )}
                {student.gender && (
                ...
                {age !== null && (
                  <span className="flex items-center gap-2 px-3 py-2 bg-green-50 text-green-700 rounded-lg font-medium">
                    🎂 {age} years
                  </span>
                )}
              </div>
          ...
          <div className="bg-[var(--white)] rounded-xl shadow-sm p-6 border border-[var(--gray-200)]">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-[var(--text-main)] font-medium">{student.email}</p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-muted)] uppercase mb-1">
                  Phone Number
          ...
          <div className="bg-[var(--white)] rounded-xl shadow-sm p-6 border border-[var(--gray-200)]">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-[var(--text-main)] font-medium font-mono text-sm">{student.uid}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {student.about && (
          <div className="bg-[var(--white)] rounded-xl shadow-sm p-6 border border-[var(--gray-200)] mb-6">
            ...
            <p className="text-[var(--text-main)] leading-relaxed">{student.about}</p>
          </div>
        )}
